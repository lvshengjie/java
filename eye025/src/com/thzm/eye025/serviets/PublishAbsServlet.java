	...
	ServletException, IOException {
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			try {
				...
				boolean publishOk=AbstaractService.publish(content, publishStatus, p1, p2, p3);
				if(publishOk) {//简介发表成功，取出简介所有文章，并转到简介页面
					request.getRequestDispatcher("abstractList").forward(request, response);
					
				}else {
					//返回简介发表页面，并提示发表错误
					request.setAttribute("msg", "医院简介发表错误，请重试或联系管理员");
					request.getRequestDispatcher("abstrace.jsp").forward(request, response);
				}
